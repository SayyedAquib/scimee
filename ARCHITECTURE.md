...
- **Direct WOFF2 Font Preloading & Urdu Subsetting**: Primary Latin `Plus Jakarta Sans` WOFF2 font file is preloaded via `<link rel="preload" as="font" type="font/woff2" crossorigin>` and defined via inlined `@font-face` with `font-display: swap`. The cultural Urdu tagline font (`Noto Nastaliq Urdu`) uses Google Fonts dynamic `&text=` micro-subsetting loaded asynchronously, shrinking the Urdu font payload from 169.7 KiB down to ~3 KiB (98% reduction).
- **Third-Party Script Deferral**: Google Analytics 4 (`gtag.js`) is deferred to the end of `<body>`, freeing the main thread from long-task contention during the initial paint cycle.
- **True Deferred Below-The-Fold Mounting**: In addition to `React.lazy()` and `<Suspense>`, below-the-fold sections (`SyllabusExplorer`, `FacilitiesSection`, `CbtShowcaseSection`, `MapLocation`, `FAQSection`, `Footer`) are deferred until user interaction (scroll, touch, mouse, keydown) or a 2,500 ms `requestIdleCallback`. This drops initial mobile Time to Interactive (TTI) to ~1.2s and collapses Total Blocking Time (TBT) to < 50ms.
- **Scheduler Chunk Deduplication**: `manualChunks` in `vite.config.js` unifies `scheduler` into `vendor-react`, eliminating split vendor micro-chunks and saving 259 ms of main-thread execution on mobile CPUs.
